...
class DockerComposeGeneratorSpec extends Specification {
    ...
    def "docker-compose.yml is created at the output root"() {
        when:
        generator.generate([order], outputRoot, FractalxConfig.defaults())
    ...
    def "generated services depend on fractalx-registry being healthy"() {
        when:
        generator.generate([order, payment], outputRoot, FractalxConfig.defaults())
    ...
    def "saga orchestrator service is never included (withdrawn from stable core)"() {
        when:
        generator.generate([order], outputRoot, FractalxConfig.defaults())
    ...
    def "admin-service is never included (withdrawn from stable core)"() {
        when:
        generator.generate([order], outputRoot, FractalxConfig.defaults())

        then:
        !compose().contains("admin-service:")
    }

    def "fractalx-gateway is always included"() {
        when:
        generator.generate([order], outputRoot, FractalxConfig.defaults())

        then:
        compose().contains("fractalx-gateway:")
    }
